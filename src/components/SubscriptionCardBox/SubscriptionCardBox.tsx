...
export default function SubscriptionCardBox({
  title = "",
  descriptions = "",
  bullets = [],
  price = 0,
  ...
  return (
    <article
      className={[
        "group relative h-[515px] flex flex-col justify-between w-full overflow-hidden rounded-3xl shadow-[0_12px_32px_rgba(3,18,47,0.12)] transition",
        active
    ...
    >
      {/* crown badge */}
        ...
        <div className=" h-auto m-4 flex flex-col items-center">
          <h4 className="font-bold font-sm mt-6">{title}</h4>
          <div className="mx-8 mt-4">
            <p>{descriptions}</p>
            <ul>
              {bullets.map((bullet, index) => (
                <li key={index} className={`mt-3 flex items-start text-sm ${active ? "text-white" : "text-slate-600"}`}>
                  <svg viewBox="0 0 24 24" className={`h-4 w-4 flex-shrink-0 text-[#3B5BFF] mr-2 ${active ? "text-white" : "text-slate-600"}`} fill="none" aria-hidden>
      ...
      <div className={["mt-6 flex items-center justify-between rounded-b-3xl px-5 py-4", active ? "bg-[#2E51E6] border-t-[0.8px] border-white/60" : "bg-white/70  border-t-[0.8px] border-black/10"].join(" ")}>
        <div>
          <h3 className="text-[15px] font-semibold leading-tight">${price}</h3>
